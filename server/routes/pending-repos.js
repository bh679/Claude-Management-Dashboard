...
router.get('/', (req, res) => {
  try {
    const contentJson = execSync(
      'gh api repos/bh679/coo-agent/contents/pending-repos.json',
      { encoding: 'utf8', timeout: 15000 }
    );
    const contentData = JSON.parse(contentJson);
    const pendingRepos = JSON.parse(Buffer.from(contentData.content, 'base64').toString('utf8'));

    res.json(pendingRepos);
  } catch (err) {
